...
export async function toolsProvider(ctl: ToolsProviderController): Promise<Tool[]> {
  function cfg(): { root: string; useMcp: boolean; openwolfCmd: string } {
    let projectRoot = "", useMcp = false, openwolfCmd = "openwolf";
    try {
    ...
    return { root: resolveProjectRoot(projectRoot, workingDir), useMcp, openwolfCmd };
  }
  ...
  const recallTool = tool({
    ...
    implementation: async ({ query }) => {
      const { root: r, useMcp, openwolfCmd } = cfg();
